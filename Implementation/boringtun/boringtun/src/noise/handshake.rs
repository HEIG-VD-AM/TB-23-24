...
use crate::smartcard::{extended_diffie_hellman, extended_public_key_derivation};
...
pub fn parse_handshake_anon(
    ...
    let ephemeral_shared = extended_diffie_hellman(&static_private, &peer_ephemeral_public);
...
impl NoiseParams {
    ...
    fn new(
        ...
        let static_shared = extended_diffie_hellman(&static_private, &peer_static_public);
    ...
    fn set_static_private(
        ...
        let check_key = extended_public_key_derivation(&static_private);
        ...
        self.static_shared = extended_diffie_hellman(&self.static_private, &self.peer_static_public);
    }
}
...
impl Handshake {
    ...
    pub(super) fn receive_handshake_initialization<'a>(
        ...
        let ephemeral_shared = extended_diffie_hellman(&self.params.static_private, &peer_ephemeral_public);
        let temp = b2s_hmac(&chaining_key, &ephemeral_shared.to_bytes());
        // initiator.chaining_key = HMAC(temp, 0x1)
        chaining_key = b2s_hmac(&temp, &[0x01]);
        // key = HMAC(temp, initiator.chaining_key || 0x2)
    ...
    pub(super) fn receive_handshake_response(
        ...
        let temp = b2s_hmac(
            ...
            //    .diffie_hellman(&unencrypted_ephemeral)
            //    .to_bytes(),
            extended_diffie_hellman(&self.params.static_private, &unencrypted_ephemeral).as_bytes(),
        );
        // responder.chaining_key = HMAC(temp, 0x1)
        chaining_key = b2s_hmac(&temp, &[0x01]);
    ...
    pub(super) fn format_handshake_initiation<'a>(
        ...
        aead_chacha20_seal(
            ...
            &key,
            0,
            self.params.static_public.as_bytes(), // TODO : A modifier à l'origine
            &hash,
        );
