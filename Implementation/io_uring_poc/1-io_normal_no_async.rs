...
// Programme de test pour la partie `Performances` utilisant des IO classiques et n'utilisant pas de mécanisme asynchrone
...
impl<'a> Connection<'a> {
    ...
    pub fn respond(&mut self, nic: &mut Iface) -> std::io::Result<usize> {
        ...
        let nbytes = nic.send(&buf[..buf.len() - unwritten])?;
