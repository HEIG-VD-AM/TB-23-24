...
// Programme de test pour la partie `Performances` utilisant io_uring et un mécanisme asynchrone
...
impl Connection {
    pub fn start(iph: etherparse::Ipv4HeaderSlice, data: Arc<[u8]>) -> std::io::Result<Option<Self>> {
        let c = Connection {
    ...
    pub async fn respond(&mut self, file: &Arc<Mutex<BufferedFile>>) -> std::io::Result<usize> {
        let mut buf = [0u8; 1500];

        let _ = self.ip.set_payload_len(ICMP_PACKET_SIZE - 20);
        ...
        let nbytes = file.lock().unwrap().write_at(buf[..buf.len() - unwritten].to_vec(), 0).await.unwrap();
    ...
    local_ex.run(async {
        ...
        for _ in 0..30 {
            let handle = glommio::spawn_local(async move {
                let file_read = Arc::new(Mutex::new(unsafe { BufferedFile::from_raw_fd(fd) }));
                let file_write = Arc::new(Mutex::new(unsafe { BufferedFile::from_raw_fd(fd) }));
                loop {
                    let buf = file_read.lock().unwrap().read_at(0, ICMP_PACKET_SIZE).await.unwrap();
                    let nbytes = buf.len();
                    ...
                    match etherparse::Ipv4HeaderSlice::from_slice(&buf[..nbytes]) {
                        Ok(iph) => {
                            ...
                            let data_buf = Arc::from(buf[iph.slice().len()..nbytes].to_vec());
                            let file_write = Arc::clone(&file_write);

                            if let Some(mut c) = Connection::start(iph, data_buf).unwrap() {
                                let handle = glommio::spawn_local(async move {
                                    c.respond(&file_write).await.unwrap();
                                });
                                handle.await;
                            }
                        }
